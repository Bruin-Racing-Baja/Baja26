...
ODrive odrive_ecvt(&bus, ODRIVE_NODE_ID);
ODrive odrive_ecent(&bus, ODRIVE_NODE_ID);
Actuator actuator_ecvt(&odrive_ecvt);
Actuator actuator_ecent(&odrive_ecent);
File log_file;
...
void on_engage_limit_switch() {
  ...
  float vel_estimate = odrive_ecvt.get_vel_estimate();
  if (vel_estimate < -10) {
    odrive_ecvt.set_axis_state(ODrive::AXIS_STATE_IDLE);
  }
}
...
*/
  ...
  control_state.inbound_limit_switch = actuator_ecvt.get_inbound_limit();
  can_buffer.push(create_can_msg(CAN_ECVT_INBOUND_LIMIT_SWITCH, RASP_NODE_ID, sync_val, (u32) actuator_ecvt.get_inbound_limit())); 

  control_state.outbound_limit_switch = actuator_ecvt.get_outbound_limit();
  can_buffer.push(create_can_msg(CAN_ECVT_OUTBOUND_LIMIT_SWITCH, RASP_NODE_ID, sync_val, (u32) actuator_ecvt.get_outbound_limit())); 

  control_state.engage_limit_switch = actuator_ecvt.get_engage_limit();
  can_buffer.push(create_can_msg(CAN_ECVT_ENGAGE_LIMIT_SWITCH, RASP_NODE_ID, sync_val, (u32) actuator_ecvt.get_engage_limit())); 

  control_state.last_heartbeat_ms = odrive_ecvt.get_time_since_heartbeat_ms();
  can_buffer.push(create_can_msg(CAN_ECVT_LAST_HEARTBEAT_MS, RASP_NODE_ID, sync_val, odrive_ecvt.get_time_since_heartbeat_ms())); 

  control_state.disarm_reason = odrive_ecvt.get_disarm_reason();
  can_buffer.push(create_can_msg(CAN_ECVT_DISARM_REASON, RASP_NODE_ID, sync_val, odrive_ecvt.get_disarm_reason())); 

  control_state.active_errors = odrive_ecvt.get_active_errors();
  can_buffer.push(create_can_msg(CAN_ECVT_ACTIVE_ERRORS, RASP_NODE_ID, sync_val, odrive_ecvt.get_active_errors())); 

  control_state.procedure_result = odrive_ecvt.get_procedure_result();
  can_buffer.push(create_can_msg(CAN_ECVT_PROCEDURE_RESULT, RASP_NODE_ID, sync_val, (u32) odrive_ecvt.get_procedure_result())); 

  control_state.bus_current = odrive_ecvt.get_bus_current();
  can_buffer.push(create_can_msg(CAN_ECVT_BUS_CURRENT, RASP_NODE_ID, sync_val, odrive_ecvt.get_bus_current())); 

  control_state.bus_voltage = odrive_ecvt.get_bus_voltage();
  can_buffer.push(create_can_msg(CAN_ECVT_BUS_VOLTAGE, RASP_NODE_ID, sync_val, odrive_ecvt.get_bus_voltage())); 

  control_state.iq_measured = odrive_ecvt.get_iq_measured();
  can_buffer.push(create_can_msg(CAN_ECVT_IQ_MEASURED, RASP_NODE_ID, sync_val, odrive_ecvt.get_iq_measured())); 

  control_state.iq_setpoint = odrive_ecvt.get_iq_setpoint();
  can_buffer.push(create_can_msg(CAN_ECVT_IQ_SETPOINT, RASP_NODE_ID, sync_val, odrive_ecvt.get_iq_setpoint())); 

  control_state.velocity_estimate = odrive_ecvt.get_vel_estimate();
  can_buffer.push(create_can_msg(CAN_ECVT_VELOCITY_ESTIMATE, RASP_NODE_ID, sync_val, odrive_ecvt.get_vel_estimate())); 

  control_state.position_estimate = odrive_ecvt.get_pos_estimate();
  can_buffer.push(create_can_msg(CAN_ECVT_POSITION_ESTIMATE, RASP_NODE_ID, sync_val, odrive_ecvt.get_pos_estimate())); 
...
void button_shift_mode() {
  ...
  Serial.printf("State: %d, Velocity: %f, Out: %d, Engage: %d, In: %d,\n",
                odrive_ecvt.get_axis_state(), odrive_ecvt.get_vel_estimate(),
                actuator_ecvt.get_outbound_limit(), actuator_ecvt.get_engage_limit(),
                actuator_ecvt.get_inbound_limit());

  float velocity = 10.0;
  if (button_pressed[0]) {
    odrive_ecvt.set_axis_state(ODrive::AXIS_STATE_IDLE);
  } else if (button_pressed[1]) {
    odrive_ecvt.set_axis_state(ODrive::AXIS_STATE_CLOSED_LOOP_CONTROL);
  } else if (button_pressed[2]) {
    actuator_ecvt.set_velocity(-velocity);
  } else if (button_pressed[3]) {
    actuator_ecvt.set_velocity(0.0);
...
void setup() {
  ...
  bus.set_odrive_ecent(&odrive_ecent);
  bus.set_odrive_ecvt(&odrive_ecvt);
  ...
  if (wait_for_can) {
    ...
    while (odrive_ecvt.get_time_since_heartbeat_ms() > 100) {
      write_all_leds(millis() % (led_flash_time_ms * 2) < led_flash_time_ms);
      delay(100);
    }
  }
